import { detectCompanyType } from "./company-type.js";
...
export const BANK_FIELD_KEYS = Object.freeze([
  "netProfit",
  "totalAssets",
  "equity",
  "previousAssets",
...
const COMPANY_MARKETS = Object.freeze({
  saudi: Object.freeze({
    id: "saudi",
    code: "SA",
    label: "تداول",
...
export function normalizeCompanySymbol(value) {
  return detectCompanyMarket(value)?.yahooSymbol || null;
}

export function normalizeSaudiSymbol(value) {
  const detected = detectCompanyMarket(value);
  return detected?.id === "saudi" ? detected.yahooSymbol : null;
}
...
function completenessScore(statement, companyType) {
  const fields = companyType === "bank"
    ? [
      statement.totalAssets,
      firstFinite(statement.stockholdersEquity, statement.commonStockEquity),
      firstFinite(statement.netIncome, statement.netIncomeCommonStockholders),
    ]
    : [
  ...
  return fields.filter((value) => typeof value === "number" && Number.isFinite(value)).length;
}

export function buildCompanyPayload(symbol, quoteSummary, annualSeries, dividendEvents = []) {
  const detectedMarket = detectCompanyMarket(symbol);
  const price = quoteSummary?.price ?? {};
  const assetProfile = quoteSummary?.assetProfile ?? {};
  const companyType = detectCompanyType({
    symbol,
    sector: assetProfile.sectorDisp || assetProfile.sector,
    industry: assetProfile.industryDisp || assetProfile.industry,
  });
  const statements = (Array.isArray(annualSeries) ? annualSeries : [])
    .filter((statement) => asIsoDate(statement?.date))
    .sort((a, b) => new Date(b.date) - new Date(a.date));

  const minimumScore = companyType === "bank" ? 3 : 4;
  const latest = statements.find((statement) => completenessScore(statement, companyType) >= minimumScore);
  ...
  const fields = {
    ...
    netInterestIncome: firstFinite(latest.netInterestIncome, latest.netInterestIncomeAfterProvision),
    averageEarningAssets: firstFinite(latest.averageInterestEarningAssets, latest.averageEarningAssets),
    operatingExpenses: absoluteFinite(
      latest.operatingExpense,
      latest.totalOperatingExpenses,
      latest.nonInterestExpense,
    ),
    operatingIncome: firstFinite(latest.operatingRevenue, latest.totalRevenue),
    totalLoans: firstFinite(latest.grossLoans, latest.netLoans, latest.totalLoans, latest.loansReceivable),
    customerDeposits: firstFinite(latest.customerDeposits, latest.totalDeposits, latest.deposits),
    nonPerformingLoans: firstFinite(latest.nonPerformingLoans, latest.nonPerformingAssets),
    loanLossProvisions: absoluteFinite(latest.provisionForLoanLosses, latest.creditLossesProvision),
    regulatoryCapital: firstFinite(latest.regulatoryCapital, latest.totalCapital),
    riskWeightedAssets: firstFinite(latest.riskWeightedAssets),
  };

  const applicableFieldKeys = companyType === "bank" ? BANK_FIELD_KEYS : FINANCIAL_FIELD_KEYS;
  const missingFields = applicableFieldKeys.filter((key) => fields[key] === null);
  const companyInfo = {
  ...
  return {
    ...
    market: detectedMarket ? {
      id: detectedMarket.id,
      code: detectedMarket.code,
    ...
    currency: price.currency || detectedMarket?.defaultCurrency || "SAR",
    currencySymbol: price.currencySymbol || "",
    period: latestDate.slice(0, 4),
    periodEnd: latestDate,
    fields,
    availableFieldCount: applicableFieldKeys.length - missingFields.length,
    totalFieldCount: applicableFieldKeys.length,
    missingFields,
    companyInfo,
