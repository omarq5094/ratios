import { detectCompanyMarket } from "./financial-normalizer.js";
...
function plainDescription(payload) {
  const identity = companyIdentity(payload.symbol);
  ...
  if (payload.companyType === "bank") {
    return `تحليل النسب المالية لبنك ${name}، رمز ${symbol} في ${identity.label}: التقييم والعائد والمؤشرات المصرفية وسجل التوزيعات وفق أحدث بيانات سنوية متاحة.`;
  }
  return `تحليل النسب المالية لشركة ${name}، رمز ${symbol} في ${identity.label}: الربحية والسيولة والمديونية والتقييم وسجل التوزيعات وفق أحدث بيانات سنوية متاحة.`;
}
...
export function renderCompanyErrorDocument(template, rawSymbol, message, status = 404) {
  const identity = companyIdentity(rawSymbol);
  const symbol = identity.displaySymbol;
  const canonical = symbol === "غير معروف"
    ? `${SITE_URL}/services/financial-ratios`
    : `${SITE_URL}/company/${encodeURIComponent(symbol)}`;
